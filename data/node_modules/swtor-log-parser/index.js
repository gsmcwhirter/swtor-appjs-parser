      ...
      function afterLatestFile(){
        ...
        if (self.latest_file_mtime){
          ...
          function processItems(next){
            ...
            if (stats.isFile() && (!new_latest_mtime || stats.mtime.getTime() > new_latest_mtime || stats.ctime.getTime() > new_latest_mtime)){
              //we read from the beginning of that file
              var new_time = stats.mtime.getTime() > stats.ctime.getTime() ? stats.mtime.getTime() : stats.ctime.getTime();
              self.readFile(item, 0, function (position){
                new_latest_mtime = new_time
